-- Cultivest Database Schema V3 - Multi-Chain Investment Platform with Solana
-- This consolidates all changes through Solana support (Asset Type 4)
...
-- 1.1 Users table (consolidating custom + auth fields + Solana balance)
...
-- 1.2 Wallets table (tri-chain support: Bitcoin + Algorand + Solana)
CREATE TABLE IF NOT EXISTS wallets (
  wallet_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,
  
  -- Blockchain type
  blockchain VARCHAR(20) DEFAULT 'multi-chain' CHECK (blockchain IN ('algorand', 'bitcoin', 'solana', 'multi-chain')),
  ...
  -- Solana wallet (Asset Type 4)
  solana_address VARCHAR(44), -- Solana addresses are 32-44 chars (base58)
  encrypted_solana_private_key TEXT,
  balance_sol DECIMAL(18,9) DEFAULT 0, -- SOL has 9 decimal places
...
-- 2.1 Investments table (Bitcoin + Algorand + Solana investments)
CREATE TABLE IF NOT EXISTS investments (
  ...
  target_asset VARCHAR(10) NOT NULL CHECK (target_asset IN ('BTC', 'ALGO', 'USDC', 'SOL')),
  amount_usd DECIMAL(18,2) NOT NULL,
  
  -- Asset estimates (tri-chain)
  estimated_btc DECIMAL(18,8),
  estimated_algo DECIMAL(18,6),
  estimated_sol DECIMAL(18,9),
  bitcoin_price_usd DECIMAL(18,2),
  algo_price_usd DECIMAL(18,6),
  solana_price_usd DECIMAL(18,6),
...
-- 2.3 Deposits table (MoonPay + tri-chain currency)
CREATE TABLE IF NOT EXISTS deposits (
  deposit_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,
  wallet_id UUID REFERENCES wallets(wallet_id) ON DELETE SET NULL,
  
  -- Amount details (tri-chain)
  amount_usd DECIMAL(18,2) NOT NULL,
  ...
  -- Target details
  target_currency VARCHAR(10) DEFAULT 'algo' CHECK (target_currency IN ('btc', 'algo', 'usdca', 'sol')),
  ...
  -- Processing details (tri-chain status)
...
CREATE TABLE IF NOT EXISTS position_nfts (
  ...
  -- Position details (tri-chain support)
  blockchain VARCHAR(20) NOT NULL,
  cryptocurrency VARCHAR(10) NOT NULL CHECK (cryptocurrency IN ('BTC', 'ALGO', 'USDC', 'SOL')),
...
-- Insert default badges (tri-chain focused)
...
-- Insert sample educational content (tri-chain)
INSERT INTO educational_content (type, title, description, url, duration_seconds, difficulty_level, is_published, quiz_questions) VALUES
  ...
  ('video', 'Solana Fundamentals', 'Introduction to Solana blockchain and its ecosystem', 'https://example.com/solana-basics', 300, 'beginner', true, NULL),
  ...
  ('quiz', 'Solana Knowledge Check', 'Test your understanding of Solana blockchain technology', NULL, NULL, 'beginner', true,
   '[
     {
       "question": "What consensus mechanism does Solana use?",
       "options": ["Proof of Work", "Proof of Stake", "Proof of History + Proof of Stake", "Delegated Proof of Stake"],
       "correct_answer": 2
     },
     {
       "question": "What is SOL?",
       "options": ["A stablecoin", "Solana''s native token", "A smart contract language", "A wallet application"],
       "correct_answer": 1
     }
   ]'::jsonb)
...
COMMENT ON TABLE users IS 'Core user accounts with tri-chain wallet support (Bitcoin + Algorand + Solana)';
COMMENT ON TABLE wallets IS 'Tri-chain wallet storage with encrypted private keys for Bitcoin, Algorand, and Solana';
COMMENT ON TABLE investments IS 'Bitcoin, Algorand, and Solana investment tracking (Asset Types 1-4)';
COMMENT ON TABLE investment_positions IS 'Stablecoin yield farming positions';
COMMENT ON TABLE deposits IS 'MoonPay fiat-to-crypto deposits with tri-chain currency support';
COMMENT ON TABLE portfolio_nfts IS 'NFT-based portfolio representation on Algorand with tri-chain holdings';
COMMENT ON TABLE position_nfts IS 'Individual investment position NFTs supporting Bitcoin, Algorand, USDC, and Solana';
COMMENT ON TABLE user_nft_portfolios IS 'Maps users to their portfolio NFT tokens for custodial asset management';

-- Tri-chain specific field comments
COMMENT ON COLUMN users.current_balance_sol IS 'User''s current SOL balance (9 decimal places)';
...
COMMENT ON COLUMN deposits.amount_sol IS 'SOL amount in deposit (9 decimal places)';
COMMENT ON COLUMN portfolio_nfts.sol_holdings IS 'SOL holdings in portfolio (9 decimal places)';
...
-- Database schema V3 creation complete!
-- This includes full tri-chain support: Bitcoin (Asset Type 1) + Algorand (Asset Type 2) + USDC (Asset Type 3) + Solana (Asset Type 4)
-- Multi-chain wallet, investment, deposit, and NFT portfolio support
-- Ready for production deployment with comprehensive Solana integration 